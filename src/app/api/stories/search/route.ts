import type { PostConnection } from "@/interfaces";
...
const GET_SEARCH_RESULTS = gql`
  query getPostsSearchResults($first: Int, $last: Int, $after: String, $before: String, $where: RootQueryToPostConnectionWhereArgs) {
    posts (
      first: $first,
      last: $last,
      after: $after,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = response?.data?.posts && {
      contentNodes: response.data.posts.nodes,
    };
