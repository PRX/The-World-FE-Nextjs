...
export async function generateMetadata(
  ...
  const data = await getCachedRegion(slug);
  ...
  const seoLink = `https://theworld.org/tags/regions/${slug}`;
  const md = {
    ...(seo || {
      title: name,
...
export default async function RegionPage({ params, searchParams }: Props) {
  ...
  let data: Awaited<ReturnType<typeof getCachedRegion>>;

  if (slug && !isTaxonomySlug) {
    data = await getCachedRegion(slug);
  ...
  const searchFilters = {
    ...
    ctx: create(TaxonomyContextSchema, {
      taxonomy: SFTaxonomyEnum.region,
      termSlug: slug,
    }),
    ...(!!excludeIds?.length && {
  ...
  const contentData = await getCachedRegionContent(
    {
      first: 60,
      where: {
        search,
