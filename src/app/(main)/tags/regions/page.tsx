...
import type { Region, Episode, Post, Segment } from "@/interfaces";
import { unstable_cache } from "next/cache";
import Image from "next/image";
import { isArray } from "lodash";
import CtaRegion from "@/app/(main)/_components/CtaRegion";
import { getCtaRegionMessages, getShownMessage } from "@/lib/cta";
import { fetchGqlRegions, type RegionsQueryOptions } from "@/lib/fetch";
...
export const getCachedRegions = unstable_cache(
  async (query: RegionsQueryOptions) => fetchGqlRegions(query),
  ["regions"],
  {
...
export async function generateMetadata(
  ...
  const seoTitle = "Regions";
  const link = "https://theworld.org/tags/regions";
  const md = {
    canonical: link,
    title: seoTitle,
...
export default async function RegionsPage({
  ...
  const data = await getCachedRegions({
    first: 20,
    where: {
      search,
    },
