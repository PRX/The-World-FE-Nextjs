...
import type { SocialTag, Episode, Post, Segment } from "@/interfaces";
import { unstable_cache } from "next/cache";
import Image from "next/image";
import { isArray } from "lodash";
import CtaRegion from "@/app/(main)/_components/CtaRegion";
import { getCtaRegionMessages, getShownMessage } from "@/lib/cta";
import { fetchGqlSocialTags, type SocialTagsQueryOptions } from "@/lib/fetch";
...
import { BookmarkIcon, HashIcon } from "lucide-react";
...
export const getCachedSocialTags = unstable_cache(
  async (query: SocialTagsQueryOptions) => fetchGqlSocialTags(query),
  ["socialTags"],
  {
...
export async function generateMetadata(
  ...
  const seoTitle = "Social Tags";
  const link = "https://theworld.org/tags/social_tags";
  const md = {
    canonical: link,
    title: seoTitle,
...
export default async function SocialTagsPage({
  searchParams,
}: {
  ...
  const data = await getCachedSocialTags({
    first: 20,
    where: {
      search,
    },
