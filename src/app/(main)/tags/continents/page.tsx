...
import type { Continent, Episode, Post, Segment } from "@/interfaces";
import { unstable_cache } from "next/cache";
import Image from "next/image";
import { isArray } from "lodash";
import CtaRegion from "@/app/(main)/_components/CtaRegion";
import { getCtaRegionMessages, getShownMessage } from "@/lib/cta";
import { fetchGqlContinents, type ContinentsQueryOptions } from "@/lib/fetch";
...
export const getCachedContinents = unstable_cache(
  async (query: ContinentsQueryOptions) => fetchGqlContinents(query),
  ["continents"],
  {
...
export async function generateMetadata(
  ...
  const seoTitle = "Continents";
  const link = "https://theworld.org/tags/continents";
  const md = {
    canonical: link,
    title: seoTitle,
...
export default async function ContinentsPage({
  ...
  const data = await getCachedContinents({
    first: 20,
    where: {
      search,
    },
