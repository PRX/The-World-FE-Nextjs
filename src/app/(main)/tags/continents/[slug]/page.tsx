...
export async function generateMetadata(
  ...
  const data = await getCachedContinent(slug);
  ...
  const seoLink = `https://theworld.org/tags/continents/${slug}`;
  const md = {
    ...(seo || {
      title: name,
  ...
  // console.log("CONTINENT SEO", seo, md);

  return convertSeoToMetadata(md, metadata) || {};
}

export default async function ContinentPage({ params, searchParams }: Props) {
  const { slug } = await params;
  const resolvedSearchParams = await searchParams;
  const isTaxonomySlug = taxonomySlugToSingularName.has(slug);
  let data: Awaited<ReturnType<typeof getCachedContinent>>;

  if (slug && !isTaxonomySlug) {
    data = await getCachedContinent(slug);
  ...
  const searchFilters = {
    ...
    ctx: create(TaxonomyContextSchema, {
      taxonomy: SFTaxonomyEnum.continent,
      termSlug: slug,
    }),
    ...(!!excludeIds?.length && {
  ...
  const contentData = await getCachedContinentContent(
    {
      first: 60,
      where: {
        search,
