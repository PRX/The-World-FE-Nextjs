...
import type { ProvinceOrState, Episode, Post, Segment } from "@/interfaces";
...
import {
  fetchGqlProvincesOrStates,
  type ProvincesOrStatesQueryOptions,
} from "@/lib/fetch";
...
export const getCachedProvincesOrStates = unstable_cache(
  async (query: ProvincesOrStatesQueryOptions) =>
    fetchGqlProvincesOrStates(query),
  ["provincesOrStates"],
  {
...
export default async function ProvincesOrStatesPage({
  ...
  const data = await getCachedProvincesOrStates({
    first: 20,
    where: {
      search,
    },
