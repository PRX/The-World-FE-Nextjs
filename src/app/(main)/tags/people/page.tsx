...
import type { Person, Episode, Post, Segment } from "@/interfaces";
import { unstable_cache } from "next/cache";
import Image from "next/image";
import { isArray } from "lodash";
import CtaRegion from "@/app/(main)/_components/CtaRegion";
import { getCtaRegionMessages, getShownMessage } from "@/lib/cta";
import { fetchGqlPeople, type PeopleQueryOptions } from "@/lib/fetch";
...
export const getCachedPeople = unstable_cache(
  async (query: PeopleQueryOptions) => fetchGqlPeople(query),
  ["people"],
  {
...
export async function generateMetadata(
  ...
  const seoTitle = "People";
  const link = "https://theworld.org/tags/people";
  const md = {
    canonical: link,
    title: seoTitle,
...
export default async function PeoplePage({
  ...
  const data = await getCachedPeople({
    first: 20,
    where: {
      search,
    },
